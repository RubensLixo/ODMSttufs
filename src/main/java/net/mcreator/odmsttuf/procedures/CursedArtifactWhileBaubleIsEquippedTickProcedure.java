			...
			if (entity instanceof LivingEntity lv ? CuriosApi.getCuriosHelper().findEquippedCurio(OdmsttufModItems.CURSED_ARTIFACT.get(), lv).isPresent() : false) {
				...
				if (OdmsttufModVariables.MapVariables.get(world).ativadoheart == 1) {
							...
							for (Entity entityiterator : _entfound) {
								if (!(entityiterator.getDisplayName().getString()).equals("Dev")) {
									if (!(entity instanceof ItemEntity)) {
										entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("odmsttuf:abyssdevours")))),
												1);
									}
								}
							}
					...
					if (OdmsttufModVariables.MapVariables.get(world).ImortalidadeCondenada == 1) {
						...
						if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.forceAddEffect(new MobEffectInstance(MobEffects.REGENERATION, 60, 10, false, false), entity);
						if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
